...
def fetchDownload(project_id,file_id,amp_instance):
    ...
    file_path = f'/server/{amp_instance}/Minecraft/downloads/{file_id}.zip'
...
def fetchPlugin(id,amp_instance):
    ...
    with open(f'/server/{amp_instance}/Minecraft/downloads/{file_name}', 'wb') as file:
        file.write(download.content)
    logging.debug(f'Downloaded {file_name} from {latest_file_url}')
    sha1 = hashlib.sha1()
    try:
        with open(f'/server/{amp_instance}/Minecraft/downloads/{file_name}', 'rb') as file:
            # Read the file in chunks to avoid memory issues with large files
            while chunk := file.read(8192):
                sha1.update(chunk)
    except FileNotFoundError:
    ...
    if file_hash == file_hash_sha1:
        ...
        logging.error('File hash does not match')
        if os.path.exists(f'/server/{amp_instance}/Minecraft/downloads/{file_name}'):
            os.remove(f'/server/{amp_instance}/Minecraft/downloads/{file_name}')
        return
    logging.debug('Installing plugin')
    if not os.path.exists(f'/server/{amp_instance}/Minecraft/plugins/{file_name}'):
        os.makedirs(f'/server/{amp_instance}/Minecraft/plugins', exist_ok=True)
        shutil.copy(f'/server/{amp_instance}/Minecraft/downloads/{file_name}', f'/server/{amp_instance}/Minecraft/plugins/{file_name}')
...
def installFiles(amp_instance,file_path):
    ...
    os.makedirs(f'/server/{amp_instance}/Minecraft/temp/mods', exist_ok=True)
    os.makedirs(f'/server/{amp_instance}/Minecraft/temp/config', exist_ok=True)
    ...
    for pattern in mods_patterns:
        for file in glob.glob(f'/server/{amp_instance}/Minecraft/mods/{pattern}'):
            destination = os.path.join(f'/server/{amp_instance}/Minecraft/mods', os.path.basename(file))
            shutil.move(file, destination)

    for pattern in config_patterns:
        for file in glob.glob(f'/server/{amp_instance}/Minecraft/config/{pattern}'):
            destination = os.path.join('/server/{amp_instance}/Minecraft/config', os.path.basename(file))
            shutil.move(file, destination)

    # Delete the old mods folder and the config folder:
    logging.info('Deleting old mods and config folders')
    if os.path.exists(f'/server/{amp_instance}/Minecraft/mods'):
        os.system(f'rm -rf /server/{amp_instance}/Minecraft/mods')
    if os.path.exists('/server/{amp_instance}/Minecraft/config'):
        os.system(f'rm -rf /server/{amp_instance}/Minecraft/config')

    # Unzip the file if it's a zip file
    logging.info('Unzipping file')
    if file_path.endswith('.zip'):
        with zipfile.ZipFile(file_path, 'r') as zip_ref:
            zip_ref.extractall(f'/server/{amp_instance}/Minecraft/')
    ...
    for file in glob.glob(f'./server/{amp_instance}/Minecraft/temp/mods/*'):
        shutil.move(file, f'/server/{amp_instance}/Minecraft/mods')
    for file in glob.glob(f'/server/{amp_instance}/Minecraft/temp/config/*'):
        shutil.move(file, f'/server/{amp_instance}/Minecraft/config')

    # Remove the zip file and the temp folder
    logging.info('Removing zip file and temp folder')
    if os.path.exists(file_path):
        os.remove(file_path)
    if os.path.exists(f'/server/{amp_instance}/Minecraft/temp'):
        os.system(f'rm -rf /server/{amp_instance}/Minecraft/temp')
...
def update():
    ...
    if request.headers.get('api_key') == my_api_key:
        ...
        if not os.path.exists(f'/server/{amp_instance}/Minecraft/downloads/{server_file_id}.zip'):
            logging.info('Downloading...')
            file_path = fetchDownload(project_id,server_file_id,amp_instance)
        else:
            logging.info('File already exists')
            file_path = f'/server/{amp_instance}/Minecraft/downloads/{server_file_id}.zip'
